...
// 아이템 위치 갱신 주기 (초)
static const double ITEM_RESPAWN_INTERVAL = 5.0;

// 게이트 재생성 주기 (초)
static const double GATE_RESPAWN_INTERVAL = 10.0;

// 특수 아이템 지속 시간 (초)
static const double SPECIAL_ITEM_DURATION = 3.0;
...
void Game::spawn_items() {
    ...
    // 스테이지 1·2: 무적 아이템(8, 하늘색)
    if (current_stage == 0 || current_stage == 1) {
        place_item(8);
    }
    // 스테이지 3·4: 속도 느려지는 아이템(9, 노란색)
...
void Game::update_contents_val() {
    ...
    contents.set_item_timer(immune_remain, slow_remain);
}
...
void Game::update_timers() {
    ...
    if (current_stage == 2 || current_stage == 3) {
        ...
        static double next_collapse = WALL_COLLAPSE_INTERVAL;
    ...
    if (immune_active) {
        const double elapsed =
            std::chrono::duration<double>(now - immune_start).count();
        if (elapsed >= SPECIAL_ITEM_DURATION) {
            immune_active = false;
        }
    }
...
void Game::update_state() {
        ...
        head.second < 0 || head.second >= map.getWidth())
    {
        // 무적 상태면 반대편 벽으로 이동
        if (immune_active) {
            // 맵 바깥으로 나간 경우 반대편으로 순환
            int nx = (head.first  + map.getHeight()) % map.getHeight();
            int ny = (head.second + map.getWidth())  % map.getWidth();
            snake.set_head(nx, ny);
            head = snake.get_head();
        } else {
